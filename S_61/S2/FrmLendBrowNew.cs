...
namespace S_61.S2
{
    public partial class FrmLendBrowNew : Formbase
    {
        JBS.JS.Lend jLend;
        ...
        private void FrmLendBrowNew_Load(object sender, EventArgs e)
        {
            ...
            SQL = " 序號='',"
                + " LEFT(lendd.ledate,3)+'/'+SUBSTRING(lendd.ledate,4,2)+'/'+RIGHT(lendd.ledate,2) 借出日期,"
                + " LEFT(lendd.ledate1,4)+'/'+SUBSTRING(lendd.ledate1,5,2)+'/'+RIGHT(lendd.ledate1,2) 借出日期1,"
                + " 產品組成= case when lendd.ittrait=1 then '組合品' when lendd.ittrait=2 then '組裝品' when lendd.ittrait=3 then '單一商品' end,"
                + " lendd.*,lend.emname,lend.lememo,lend.taxmny,lend.tax,lend.totmny,lend.xa1name,lend.x3no,lend.cuname1,item.itstockqty,item.itnoudf from lendd left outer join lend "
                + " on lendd.leno = lend.leno left outer join item on lendd.itno=item.itno where '0'='0'";
        ...
        void GetTotalCount()
        {
            ...
            {
                cn.Open();
                cmd.CommandText = " Select Count(*) from lendd ";
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (qCuNo.TrimTextLenth() > 0 && qItNo.TrimTextLenth() > 0)
                {
                    ...
                    {
                        Common.Eload(dtD, SQL, "lend.CuNo", qCuNo.Text.Trim(), "lendd.Ledate");
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (qStNo.TrimTextLenth() > 0 && qItNo.TrimTextLenth() > 0)
                {
                    ...
                    if (dtD.Rows.Count < TotalCount)
                    {
                        Common.Eload(dtD, SQL, "lend.StNo", qStNo.Text.Trim(), "lendd.Ledate");
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (qEmName.TrimTextLenth() > 0 && qDate.TrimTextLenth() > 0)
                {
                    ...
                    if (dtD.Rows.Count < TotalCount)
                    {
                        Common.Eload(dtD, SQL, "lend.EmNo", qEmName.Text.Trim(), "lendd.Ledate");
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (qLeno.TrimTextLenth() > 0)
                {
                    ...
                    {
                        Common.Eload(dtD, SQL, "lend.LeNo", qLeno.Text.Trim(), "lendd.Ledate");
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (qDate.TrimTextLenth() > 0)
                {
                    ...
                    if (dtD.Rows.Count < TotalCount)
                    {
                        Common.Dload(dtD, SQL, "lendd.Ledate", date);
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (qItNo.TrimTextLenth() > 0)
                {
                    ...
                    if (dtD.Rows.Count < TotalCount)
                    {
                        Common.Eload(dtD, SQL, "lendd.ItNo", qItNo.Text.Trim(), "lendd.Ledate");
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (qMeNo.TrimTextLenth() > 0)
                {
                    ...
                    if (dtD.Rows.Count < TotalCount)
                    {
                        Common.Eload(dtD, SQL, "lendd.MeNo", qMeNo.Text.Trim(), "lendd.Ledate");
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (qUnit.TrimTextLenth() > 0)
                {
                    ...
                    if (dtD.Rows.Count < TotalCount)
                    {
                        Common.Eload(dtD, SQL, "lendd.itunit", qUnit.Text.Trim(), "lendd.Ledate");
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
        ...
        private void btnDesp_Click(object sender, EventArgs e)
        {
            ...
            using (JE.SOther.FrmDesp frm = new JE.SOther.FrmDesp(false, FormStyle.Mini))
            {
                ...
                frm.dr = Common.load("Check", "LendD", "leid", leid);
                frm.ShowDialog();
            }
        }
        ...
        private void dataGridViewT1_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlCommand cmd = cn.CreateCommand())
                {
                    ...
                    cmd.CommandText = "Update Lendd Set memo = @memo where Bomid = @Bomid";
                    cmd.ExecuteNonQuery();
                }
            }
            catch (Exception ex)
        ...
        private void LeMemo_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                using (SqlConnection cn = new SqlConnection(Common.sqlConnString))
                {
                    ...
                    using (SqlCommand cmd = cn.CreateCommand())
                    {
                        string leno = dataGridViewT1.SelectedRows[0].Cells["借出單號"].Value.ToString();
                        string memo = LeMemo.Text.Trim();
                        cmd.Parameters.AddWithValue("lememo", memo);
                        cmd.Parameters.AddWithValue("leno", leno);
                        cmd.CommandText = "update lend set lememo=@lememo where leno=@leno ";
                        cmd.ExecuteNonQuery();
                    }
                }
            }
        ...
        private void qDate_Validating(object sender, CancelEventArgs e)
        {
            ...
            jLend.DateValidate(sender, e, true); 
        }
        ...
        private void FrmLendBrowNew_Shown(object sender, EventArgs e)
        {
            qLeno.Focus();
        }
