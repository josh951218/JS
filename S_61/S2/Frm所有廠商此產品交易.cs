...
namespace S_61.S2
{
    public partial class Frm所有廠商此產品交易 : Formbase
    {
        public string fano = "";
        public string itno = "";
        ...
        private void Frm所有廠商此產品交易_Load(object sender, EventArgs e)
        {
            dtD.Clear();
            ...
            using (SqlDataAdapter da = new SqlDataAdapter(cmd))
            {
                ...
                cmd.CommandText = "Select 單據='進貨',bshopd.*,bshop.faname1 from bshopd left join bshop on bshopd.bsno=bshop.bsno where itno=(@itno)";
                da.Fill(dtD);
                cmd.CommandText = "Select 單據='退貨',rshopd.*,rshop.faname1 from rshopd left join rshop on rshopd.bsno=rshop.bsno where itno=(@itno)";
                da.Fill(dtD);
            }
            dtD.DefaultView.Sort = "bsdate DESC,fano ASC,bsno ASC";
            if (dtD.Rows.Count > 0)
            {
                dataGridViewT1.DataSource = dtD.DefaultView;
                dtD.DefaultView.Search(ref dataGridViewT1, "fano", fano);
            }
            else
            {
