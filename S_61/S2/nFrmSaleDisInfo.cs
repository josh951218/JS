...
namespace S_61.S2
{
    public partial class nFrmSaleDisInfo : Formbase
    {
        JBS.JS.SaleDis jSaleDis;

        public nFrmSaleDisInfo()
        {
            InitializeComponent();
        ...
        private void FrmSaleDisInfo_Load(object sender, EventArgs e)
        {
            //X5No.Text = "1";
        ...
        {
            jSaleDis.Open<JBS.JS.XX05>(sender);
        }
        ...
        private void X5No_Validating(object sender, CancelEventArgs e)
        {
            ...
            jSaleDis.ValidateOpen<JBS.JS.XX05>(sender, e, row =>
            {
        ...
        private void CoNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            jSaleDis.ValidateOpen<JBS.JS.Comp>(sender, e, reader =>
            {
        ...
        private void btnBrow_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    ...
                    if (X5No.TrimTextLenth() > 0)
                        sb.Append(" And saledis.X5No = (@X5No) ");
                    if (CoNo.TrimTextLenth() > 0)
                        sb.Append(" And saledis.CoNo = (@CoNo) ");
                    if (Day.TrimTextLenth() > 0)
                        sb.Append(" And saledis.DiDate >= (@Day) ");
                    if (Day1.TrimTextLenth() > 0)
                        sb.Append(" And saledis.DiDate <= (@Day1) ");
                    if (PrDate.TrimTextLenth() > 0)
                        sb.Append(" And saledis.PrDate >= (@PrDate) ");
                    if (PrDate1.TrimTextLenth() > 0)
                        sb.Append(" And saledis.PrDate <= (@PrDate1) ");
                    if (CuNo.TrimTextLenth() > 0)
                        sb.Append(" And saledis.CuNo >= (@CuNo) ");
                    if (CuNo1.TrimTextLenth() > 0)
                        sb.Append(" And saledis.CuNo <= (@CuNo1) ");
                    if (InNo.TrimTextLenth() > 0)
                        sb.Append(" And saledis.InNo >= (@InNo)");
                    if (InNo1.TrimTextLenth() > 0)
                        sb.Append(" And saledis.InNo <= (@InNo1)");
                    ...
                    cmd.CommandText = @"
                    Select 發票日期='',作廢='',saledis.* 
                    From saledisd
                    Left join saledis on saledisd.dino = saledis.dino
                    Where 0=0 " + qStr + " order by saledisd.inno";
                    da.Fill(dt);

                    sb.Clear();
                    if (X5No.TrimTextLenth() > 0)
                        sb.Append(" And saledisd.X5No = (@X5No) ");
                    if (CoNo.TrimTextLenth() > 0)
                        sb.Append(" And saledisd.CoNo = (@CoNo) ");
                    if (Day.TrimTextLenth() > 0)
                        sb.Append(" And saledisd.DiDate >= (@Day) ");
                    if (Day1.TrimTextLenth() > 0)
                        sb.Append(" And saledisd.DiDate <= (@Day1) ");
                    if (PrDate.TrimTextLenth() > 0)
                        sb.Append(" And saledisd.PrDate >= (@PrDate) ");
                    if (PrDate1.TrimTextLenth() > 0)
                        sb.Append(" And saledisd.PrDate <= (@PrDate1) ");
                    if (CuNo.TrimTextLenth() > 0)
                        sb.Append(" And saledisd.CuNo >= (@CuNo) ");
                    if (CuNo1.TrimTextLenth() > 0)
                        sb.Append(" And saledisd.CuNo <= (@CuNo1) ");
                    if (InNo.TrimTextLenth() > 0)
                        sb.Append(" And saledisd.InNo >= (@InNo)");
                    if (InNo1.TrimTextLenth() > 0)
                        sb.Append(" And saledisd.InNo <= (@InNo1)");
                    if (ItNo.TrimTextLenth() > 0)
                        sb.Append(" And ItNo >= (@ItNo)");
                    if (ItNo1.TrimTextLenth() > 0)
                        sb.Append(" And ItNo <= (@ItNo1)");
                    ...
                    cmd.CommandText = " Select * from saledisd where 0=0 " + qStr + " order by saledisd.inno";
                    da.Fill(dtD);
                }
                ...
                using (nFrmSaleDisInfob frm = new nFrmSaleDisInfob())
                {
                    dt.TableName = "saledis123";
                    dtD.TableName = "saledisd123";
