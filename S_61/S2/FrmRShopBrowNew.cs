...
namespace S_61.S2
{
    public partial class FrmRShopBrowNew : Formbase
    {
        JBS.JS.xEvents xe;
        ...
        public FrmRShopBrowNew()
        {
            ...
            this.稅前金額.Set進項金額小數();
            this.退貨數量.Set庫存數量小數();
            this.退貨日期.DataPropertyName = Common.User_DateTime == 1 ? "退貨日期" : "退貨日期1";
        ...
        private void FrmBShopBrowNew_Load(object sender, EventArgs e)
        {
            ...
            SQL = ""
+ " LEFT(RShopd.bsdate,3)+'/'+SUBSTRING(RShopd.bsdate,4,2)+'/'+RIGHT(RShopd.bsdate,2) 退貨日期,"
+ " LEFT(RShopd.bsdate1,4)+'/'+SUBSTRING(RShopd.bsdate1,5,2)+'/'+RIGHT(RShopd.bsdate1,2) 退貨日期1,"
+ " 產品組成= case when RShopd.ittrait=1 then '組合品' when RShopd.ittrait=2 then '組裝品' when RShopd.ittrait=3 then '單一商品' end,"
+ " RShopd.bsid, RShopd.bsno, RShopd.bsdate, RShopd.bsdate1, RShopd.bsdateac, RShopd.bsdateac1, RShopd.fqno,"
+ " RShopd.fano, RShopd.stno, RShopd.StName, RShopd.fono, RShopd.itno, RShopd.itname, RShopd.ittrait, RShopd.itunit,"
+ " RShopd.itpkgqty, RShopd.qty, RShopd.price, RShopd.prs, RShopd.taxprice, RShopd.mny, RShopd.priceb,"
+ " RShopd.taxpriceb, RShopd.mnyb, RShopd.memo, RShopd.bomid, RShopd.RealCost, RShopd.foid,"
+ " RShopd.Punit, RShopd.Pqty, RShopd.Pformula, RShop.faname2, RShop.faname1, RShop.fatel1, RShop.faper1,"
+ " RShop.emno, RShop.emname, RShop.spno, RShop.spname, RShop.xa1no, RShop.xa1name, RShop.xa1par,"
+ " RShop.taxmnyb, RShop.taxmny, RShop.x3no, RShop.x5no, RShop.seno, RShop.sename, RShop.x4no, RShop.x4name,"
+ " RShop.tax, RShop.totmny, RShop.taxb, RShop.totmnyb, RShop.bsmemo, RShop.bsmemo1, RShop.invno, RShop.invdate,"
+ " RShop.invdate1, RShop.invname, RShop.invtaxno, RShop.invaddr1, item.itnoudf, item.itstockqty"
+ " FROM  RShopd LEFT OUTER JOIN"
+ " RShop ON RShopd.bsno = RShop.bsno LEFT OUTER JOIN"
+ " item ON RShopd.itno = item.itno"
+ " WHERE  (0 = 0)";
        ...
        void GetTotalCount()
        {
            ...
            {
                cn.Open();
                cmd.CommandText = " Select Count(*) from RShopd ";
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (tFaNo.TrimTextLenth() > 0 && tBsdate.TrimTextLenth() > 0)
                {
                    ...
                    {
                        Common.Eload(dtD, SQL, "RShopd.FaNo", tFaNo.Text.Trim(), "RShopd.Bsdate");
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (tFoNo.TrimTextLenth() > 0 && tBsdate.TrimTextLenth() > 0)
                {
                    ...
                    if (dtD.Rows.Count < TotalCount)
                    {
                        Common.Eload(dtD, SQL, "RShopd.FoNo", tFoNo.Text.Trim(), "RShopd.Bsdate");
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (tStNo.TrimTextLenth() > 0 && tItNo.TrimTextLenth() > 0)
                {
                    ...
                    if (dtD.Rows.Count < TotalCount)
                    {
                        Common.Eload(dtD, SQL, "RShopd.StNo", tStNo.Text.Trim(), "RShopd.Bsdate");
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (tEmNo.TrimTextLenth() > 0 && tFaNo.TrimTextLenth() > 0)
                {
                    ...
                    if (dtD.Rows.Count < TotalCount)
                    {
                        Common.Eload(dtD, SQL, "RShopd.EmNo", tEmNo.Text.Trim(), "RShopd.Bsdate");
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (tBsNo.TrimTextLenth() > 0)
                {
                    ...
                    {
                        Common.Eload(dtD, SQL, "RShopd.BsNo", tBsNo.Text.Trim(), "RShopd.Bsdate");
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (tBsdate.TrimTextLenth() > 0)
                {
                    ...
                    if (dtD.Rows.Count < TotalCount)
                    {
                        Common.Dload(dtD, SQL, "RShopd.Bsdate", date);
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (tItNo.TrimTextLenth() > 0)
                {
                    ...
                    if (dtD.Rows.Count < TotalCount)
                    {
                        Common.Eload(dtD, SQL, "RShopd.ItNo", tItNo.Text.Trim(), "RShopd.Bsdate");
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (tMemo.TrimTextLenth() > 0)
                {
                    ...
                    if (dtD.Rows.Count < TotalCount)
                    {
                        Common.Eload(dtD, SQL, "RShopd.Memo", tMemo.Text.Trim(), "RShopd.Bsdate");
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
                ...
                else if (tInvNo.TrimTextLenth() > 0)
                {
                    ...
                    if (dtD.Rows.Count < TotalCount)
                    {
                        Common.Cload(dtD, SQL, "RShopd.InvNo", tInvNo.Text.Trim());
                    }
                    dataGridViewT1.DataSource = dtD.DefaultView;
        ...
        private void btnDesp_Click(object sender, EventArgs e)
        {
            ...
            using (JE.SOther.FrmDesp frm = new JE.SOther.FrmDesp(false, FormStyle.Mini))
            {
                ...
                frm.dr = Common.load("Check", "RShopd", "BsID", bsid);
                frm.ShowDialog();
            }
        }
        ...
        private void btnInv_Click(object sender, EventArgs e)
        {
            ...
            using (subMenuFm_2.FrmBShop_Inv frm = new subMenuFm_2.FrmBShop_Inv("RShop", bsno, IsPOS))
            {
        ...
        private void dataGridViewT1_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlCommand cmd = cn.CreateCommand())
                {
                    ...
                    cmd.CommandText = "Update RShopd Set memo = @memo where Bomid = @Bomid";
                    cmd.ExecuteNonQuery();
                }
            }
            catch (Exception ex)
        ...
        private void textBoxT3_Validating(object sender, CancelEventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlCommand cmd = cn.CreateCommand())
                {
                    ...
                    cmd.CommandText = "Update RShop Set BsMemo=@BsMemo where BsNo=@BsNo ";
                    cmd.ExecuteNonQuery();
                }
            }
            catch (Exception ex)
        ...
        private void labelT1_DoubleClick(object sender, EventArgs e)
        {
            ...
            var index = dataGridViewT1.Rows.GetFirstRow(DataGridViewElementStates.Selected);
            var row = Common.load("Check", "rshop", "BsNo", dtD.DefaultView[index]["BsNo"].ToString());
            if (row != null)
            {
