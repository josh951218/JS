...
namespace S_61.S2
{
    public partial class FrmRBorrow_Print_BoNo : Formbase
    {
        ...
        public FrmRBorrow_Print_BoNo()
        {
            InitializeComponent();

            this.還出日期.DataPropertyName = Common.User_DateTime == 1 ? "bodate" : "bodate1";
            this.進價.Set進貨單價小數();
            this.數量.Set庫存數量小數();
        ...
        void loadM()
        {
            ...
            try
            {
                ...
                {
                    cmd.CommandText = "select * from RBorr order by BoNo";
                    da.Fill(dt);
        ...
        private void dataGridViewT1_RowStateChanged(object sender, DataGridViewRowStateChangedEventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    ...
                    cmd.CommandText = "select 序號='',* from RBorrD where BoNo = (@BoNo) ";
                    da.Fill(dtD);
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    ...
                    cmd.CommandText = "select * from RBorr where BoNo like '%' + @BoNo + '%' order by BoNo";
                    da.Fill(dt);
                    if (dt.Rows.Count > 0)
                    {
