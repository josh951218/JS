...
namespace S_61.S2
{
    public partial class FrmBShopInv : Formbase
    {
        ...
        private void FrmBShopInv_Load(object sender, EventArgs e)
        {
            loaddtD();
            dtD.Clear();
            dtBom.Clear();

            var pk = jBShopInv.Bottom();
            writeToTxt(pk);
        }

        private void FrmBShopInv_Shown(object sender, EventArgs e)
        {
            btnAppend.Focus();
        }
        ...
        private void writeToTxt(string inno)
        {
            ...
            var result = jBShopInv.LoadData(inno, reader =>
            {
                ...
                jBShopInv.Validate<JBS.JS.XX03>(X3No.Text, r =>
                {
                ...
                jBShopInv.Validate<JBS.JS.XX05>(X5No.Text, r =>
                {
        ...
        private void loaddtBom()
        {
            ...
            try
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    cmd.Parameters.AddWithValue("@InNo", jBShopInv.GetCurrent());

                    if (FormState == FormEditState.Append)
                        cmd.CommandText = "select top 1 * from bshopinvbom where 1=0";
                    else if (FormState == FormEditState.Duplicate)
                        cmd.CommandText = "select * from bshopinvbom where InNo=@InNo ";
        ...
        {
            var pk = jBShopInv.Top();
            writeToTxt(pk);
        }

        private void btnPrior_Click(object sender, EventArgs e)
        {
            var pk = jBShopInv.Prior();
            writeToTxt(pk);
        }

        private void btnNext_Click(object sender, EventArgs e)
        {
            var pk = jBShopInv.Next();
            writeToTxt(pk);
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            using (SqlCommand cmd = cn.CreateCommand())
            {
                try
                {
                    ...
                    cmd.CommandText = "delete bshopinv where inno=@inno";
                    cmd.ExecuteNonQuery();

                    cmd.CommandText = "delete bshopinvd where inno=@inno";
                    cmd.ExecuteNonQuery();

                    cmd.CommandText = "delete bshopinvbom where inno=@inno";
                    cmd.ExecuteNonQuery();
        ...
        private void btnBrow_Click(object sender, EventArgs e)
        {
            ...
            using (var frm = new FrmBShopInvBrow())
            {
        ...
        private void InNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (jBShopInv.IsInNoFormat(sender, e) == false)
                return;

            if (FormState != FormEditState.Modify)
            {
                if (jBShopInv.IsExistDocument<JBS.JS.BShopInv>(InNo.Text.Trim()))
                {
                    e.Cancel = true;
                    MessageBox.Show(
                        "此發票號碼已重複,請檢查!",
        ...
        private void InDate_Validating(object sender, CancelEventArgs e)
        {
            ...
            jBShopInv.DateValidate(sender, e);
        }
        ...
        {
            jBShopInv.Open<JBS.JS.Fact>(sender, reader =>
            {
                FaNo.Text = reader["FaNo"].ToString().Trim();
                ...
                jBShopInv.Validate<JBS.JS.XX03>(X3No.Text, row => X3Name.Text = row["X3Name"].ToString());
                jBShopInv.Validate<JBS.JS.XX05>(X5No.Text, row => X5Name.Text = row["X5Name"].ToString());

                this.TextBefore = reader["FaNo"].ToString().Trim();
            });
        }
        ...
        private void FaNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            jBShopInv.ValidateOpen<JBS.JS.Fact>(sender, e, reader =>
            {
        ...
        {
            jBShopInv.Open<JBS.JS.XX05>(sender, row =>
            {
        ...
        private void X3No_DoubleClick(object sender, EventArgs e)
        {
            ...
            jBShopInv.Open<JBS.JS.XX03>(sender, reader =>
            {
                X3No.Text = reader["X3No"].ToString().Trim();
        ...
        private void X3No_Validating(object sender, CancelEventArgs e)
        {
            ...
            jBShopInv.ValidateOpen<JBS.JS.XX03>(sender, e, reader =>
            {
        ...
        private void dataGridViewT1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                jBShopInv.DataGridViewOpen<JBS.JS.Item>(sender, e, dtD, row => FillItem(row, e.RowIndex));
            }
            else if (CurrentColumnName == "單位")
            {
                ...
                jBShopInv.Validate<JBS.JS.Item>(itno, row =>
                {
        ...
        private void dataGridViewT1_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            ...
            else if (dataGridViewT1.Columns[e.ColumnIndex].Name == "產品編號")
            {
                ...
                jBShopInv.Validate<JBS.JS.Item>(ItNoBegin, reader =>
                {
        ...
        private void dataGridViewT1_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            if (CurrentColumnName == "產品編號")
            {
                ...
                if (ItNoNow == "" || ItNoNow.Trim().Length == 0)
                {
                    ...
                    jBShopInv.RemoveBom(rec, ref dtBom);
                    return;
                }
                //值沒變->離開
                ...
                {
                    jBShopInv.DataGridViewValidateOpen<JBS.JS.Item>(sender, e, dtD, row => FillItem(row, e.RowIndex));
                }
            }
            else if (CurrentColumnName == "單位")
            {
        ...
        private void FillItem(SqlDataReader row, int index)
        {
            ...
            var rec = dtD.Rows[index]["BomRec"].ToString().Trim();
            jBShopInv.RemoveBom(rec, ref dtBom);
            jBShopInv.GetItemBom(row["itno"].ToString().Trim(), rec, ref dtBom);
        }
        ...
        {
            jBShopInv.Open<JBS.JS.Comp>(sender, row =>
            {
        ...
        private void CoNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            jBShopInv.ValidateOpen<JBS.JS.Comp>(sender, e, row =>
            {
                CoNo.Text = row["CoNo"].ToString().Trim();
                CoName1.Text = row["coname1"].ToString().Trim();
            });
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            jBShopInv.RemoveEmptyRowOnSaving(dataGridViewT1, ref dtD, ref dtBom, SetAllMny);
            ...
            using (SqlCommand cmd = cn.CreateCommand())
            {
                ...
                try
                {
                    if (FormState == FormEditState.Append || FormState == FormEditState.Duplicate)
                    {
                        ...
                        cmd.CommandText = @"
                            insert into bshopinv 
                            (inno,indate,indate1,prdate,prdate1,cono,coname1,fano,faname1,invtaxno,x5no,x3no,invname,invaddr1,tax,taxmny,totmny,rate,inmemo,recordno,invalid,xa1par,inputday)
                            values
                            (@inno,@indate,@indate1,@prdate,@prdate1,@cono,@coname1,@fano,@faname1,@invtaxno,@x5no,@x3no,@invname,@invaddr1,@tax,@taxmny,@totmny,@rate,@inmemo,@recordno,@invalid,@xa1par,@inputday)";
                        cmd.ExecuteNonQuery();
                        ...
                        for (int i = 0; i < dtD.Rows.Count; i++)
                        {
                            ...
                            cmd.CommandText = @"
                                insert into bshopinvd 
                                (inno,indate,indate1,prdate,prdate1,cono,fano,x5no,itno,itname,itunit,punit,pqty,qty,price,rate,prs,taxprice,mny,memo,recordno,bomid,bomrec,ittrait,xa1par,priceb,taxpriceb,mnyb,itpkgqty)
                                values
                                (@inno,@indate,@indate1,@prdate,@prdate1,@cono,@fano,@x5no,@itno,@itname,@itunit,@punit,@pqty,@qty,@price,@rate,@prs,@taxprice,@mny,@memo,@recordno,@bomid,@bomrec,@ittrait,@xa1par,@priceb,@taxpriceb,@mnyb,@itpkgqty)";
                            cmd.ExecuteNonQuery();
                    ...
                    {
                        if (jBShopInv.IsExistDocument<JBS.JS.BShopInv>(jBShopInv.GetCurrent()) == false)
                        {
                        ...
                        cmd.CommandText = @"
                            Delete from bshopinvd where inno=(@inno);
                            Delete from bshopinvbom where inno=(@inno);";
                        cmd.ExecuteNonQuery();
