...
namespace S_61.S2
{
    public partial class FrmLend_Print_LeNoNew : Formbase, JBS.JS.IxOpen
    {
        ...
        private void FrmLend_Print_LeNoNew_Load(object sender, EventArgs e)
        {
            ...
            if (Common.User_DateTime == 1)
                this.借出日期.DataPropertyName = "ledate";
            else
                this.借出日期.DataPropertyName = "ledate1";
        ...
        void loadM()
        {
            try
            {
                ...
                {
                    cn.Open();
                    SqlDataAdapter dd = new SqlDataAdapter("select * from lend order by leno", cn);
                    Master.Clear();
                    list.Clear();
                    dd.Fill(Master);
        ...
        private void dataGridViewT1_SelectionChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    cn.Open();
                    string sql = "select 序號='',* from lendd where leno=@leno";
                    using (SqlCommand cmd = cn.CreateCommand())
                    {
                        cmd.Parameters.AddWithValue("leno", dataGridViewT1.SelectedRows[0].Cells["借出單號"].Value.ToString());
                        cmd.CommandText = sql;
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlConnection cn = new SqlConnection(Common.sqlConnString))
                {
                    string sql = "select * from lend where leno like '%' + @leno + '%' order by leno";
                    using (SqlCommand cmd = cn.CreateCommand())
                    {
