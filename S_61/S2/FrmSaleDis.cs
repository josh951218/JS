...
namespace S_61.S2
{
    public partial class FrmSaleDis : Formbase
    {
        ...
        private void FrmSaleDis_Load(object sender, EventArgs e)
        {
            loadSaleD();
            dtSaleD.Clear();
            dtSaleBom.Clear();

            var pk = jSaleDis.Bottom();
            writeToTxt(pk);
        }
        ...
        private void writeToTxt(string dino)
        {
            ...
            var result = jSaleDis.LoadData(dino, reader =>
            {
                ...
                jSaleDis.Validate<JBS.JS.XX03>(X3No.Text, r =>
                {
                ...
                jSaleDis.Validate<JBS.JS.XX05>(X5No.Text, r =>
                {
        ...
        private void loadSaleBom()
        {
            ...
            try
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    cmd.Parameters.AddWithValue("@DiNo", jSaleDis.GetCurrent());

                    if (FormState == FormEditState.Append)
                        cmd.CommandText = "select top 1 * from saledisbom where 1=0";
                    else if (FormState == FormEditState.Duplicate)
                        cmd.CommandText = "select * from saledisbom where DiNo=@DiNo ";
        ...
        {
            var pk = jSaleDis.Prior();
            writeToTxt(pk);
        }

        private void btnNext_Click(object sender, EventArgs e)
        {
            var pk = jSaleDis.Next();
            writeToTxt(pk);
        }
        ...
        private void WorkInit()
        {
            if (FormState == FormEditState.Append)
            {
                ...
                DiDate.Text = Date.GetDateTime(Common.User_DateTime);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            using (SqlCommand cmd = cn.CreateCommand())
            {
                try
                {
                    ...
                    cmd.CommandText = "delete saledis where DiNo=@DiNo";
                    cmd.ExecuteNonQuery();

                    cmd.CommandText = "delete saledisd where DiNo=@DiNo";
                    cmd.ExecuteNonQuery();

                    cmd.CommandText = "delete saledisbom where DiNo=@DiNo";
                    cmd.ExecuteNonQuery();
        ...
        private void btnBrow_Click(object sender, EventArgs e)
        {
            ...
            using (var frm = new FrmSaleDisBrow())
            {
        ...
        private void GetSpecialPrice(DataRow row, int index)
        {
            ...
            try
            {
                ...
                {
                    jSaleDis.Validate<JBS.JS.Item>(itno, rw =>
                    {
        ...
        private void InDate_Validating(object sender, CancelEventArgs e)
        {
            ...
            jSaleDis.DateValidate(sender, e);
        }
        ...
        {
            jSaleDis.Open<JBS.JS.Cust>(sender, reader =>
            {
        ...
        private void CuNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            jSaleDis.ValidateOpen<JBS.JS.Cust>(sender, e, reader =>
            {
                if (CuNo.Text == TextBefore)
                    return;
        ...
        {
            jSaleDis.Open<JBS.JS.XX05>(sender, row =>
            {
        ...
        {
            jSaleDis.Open<JBS.JS.XX03>(sender, reader =>
            {
        ...
        private void X3No_Validating(object sender, CancelEventArgs e)
        {
            ...
            jSaleDis.ValidateOpen<JBS.JS.XX03>(sender, e, reader =>
            {
        ...
        private void dataGridViewT1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                jSaleDis.DataGridViewOpen<JBS.JS.Item>(sender, e, dtSaleD, row => Fillitem(row, e.RowIndex));
            }
            else if (CurrentColumnName == "單位")
            {
                ...
                jSaleDis.Validate<JBS.JS.Item>(itno, row =>
                {
        ...
        private void dataGridViewT1_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            ...
            else if (dataGridViewT1.Columns[e.ColumnIndex].Name == "產品編號")
            {
                ...
                jSaleDis.Validate<JBS.JS.Item>(ItNoBegin, reader =>
                {
        ...
        private void dataGridViewT1_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            if (CurrentColumnName == "產品編號")
            {
                ...
                if (ItNoNow == "" || ItNoNow.Trim().Length == 0)
                {
                    ...
                    jSaleDis.RemoveBom(rec, ref dtSaleBom);
                    return;
                }
                //值沒變->離開
                ...
                {
                    jSaleDis.DataGridViewValidateOpen<JBS.JS.Item>(sender, e, dtSaleD, row => Fillitem(row, e.RowIndex));
                }
            }
            else if (CurrentColumnName == "單位")
            {
        ...
        private void Fillitem(SqlDataReader row, int index)
        {
            ...
            var rec = dtSaleD.Rows[index]["BomRec"].ToString().Trim();
            jSaleDis.RemoveBom(rec, ref dtSaleBom);
            jSaleDis.GetItemBom(row["itno"].ToString().Trim(), rec, ref dtSaleBom);
        }
        ...
        {
            jSaleDis.Open<JBS.JS.Comp>(sender, row =>
            {
        ...
        private void CoNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            jSaleDis.ValidateOpen<JBS.JS.Comp>(sender, e, row =>
            {
                CoNo.Text = row["CoNo"].ToString().Trim();
                CoName1.Text = row["coname1"].ToString().Trim();
            });
        ...
        private void InNo_Enter(object sender, EventArgs e)
        {
            InNo.Tag = InNo.Text.Trim();
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            jSaleDis.RemoveEmptyRowOnSaving(dataGridViewT1, ref dtSaleD, ref dtSaleBom, SetAllMny);
            ...
            using (SqlCommand cmd = cn.CreateCommand())
            {
                try
                {
                    ...
                    {
                        var result = jSaleDis.GetPkNumber<JBS.JS.SaleDis>(cmd, DiDate.Text, ref DiNo);
                        if (!result)
                        {
                            if (tn != null)
                                tn.Rollback();
                    ...
                    if (FormState == FormEditState.Append || FormState == FormEditState.Duplicate)
                    {
                        cmd.CommandText = @"
                            insert into saledis 
                            (dino,inno,didate,didate1,indate,indate1,prdate,prdate1,cono,coname1,cuno,cuname1,invtaxno,x5no,x3no,invname,invaddr1,tax,taxmny,totmny,rate,inmemo,recordno,xa1par,inputday)
                            values
                            (@dino,@inno,@didate,@didate1,@indate,@indate1,@prdate,@prdate1,@cono,@coname1,@cuno,@cuname1,@invtaxno,@x5no,@x3no,@invname,@invaddr1,@tax,@taxmny,@totmny,@rate,@inmemo,@recordno,@xa1par,@inputday)";
                        cmd.ExecuteNonQuery();
                        ...
                        for (int i = 0; i < dtSaleD.Rows.Count; i++)
                        {
                            ...
                            cmd.CommandText = @"
                                insert into saledisd 
                                (dino,inno,didate,didate1,indate,indate1,prdate,prdate1,cono,cuno,x5no,itno,itname,itunit,punit,pqty,qty,price,rate,prs,taxprice,mny,memo,recordno,bomid,bomrec,ittrait,xa1par,priceb,taxpriceb,mnyb,itpkgqty)
                                values
                                (@dino,@inno,@didate,@didate1,@indate,@indate1,@prdate,@prdate1,@cono,@cuno,@x5no,@itno,@itname,@itunit,@punit,@pqty,@qty,@price,@rate,@prs,@taxprice,@mny,@memo,@recordno,@bomid,@bomrec,@ittrait,@xa1par,@priceb,@taxpriceb,@mnyb,@itpkgqty)";
                            cmd.ExecuteNonQuery();
                    ...
                    {
                        if (jSaleDis.IsExistDocument<JBS.JS.SaleDis>(jSaleDis.GetCurrent()) == false)
                        {
                        ...
                        cmd.CommandText = @"
                            Delete from saledisd   where dino=(@dino);
                            Delete from saledisbom where dino=(@dino);";
                        cmd.ExecuteNonQuery();
            ...
            {
                using (var frm = new FrmSaleDisPrint())
                {
        ...
        private void InNo_DoubleClick(object sender, EventArgs e)
        {
            ...
            jSaleDis.Open<JBS.JS.SaleInv>(sender, reader =>
            {
                LoadSaleInv(reader);
        ...
        private void LoadSaleInv(SqlDataReader reader)
        {
            InNo.Text = reader["InNo"].ToString().Trim();
            CoNo.Text = reader["CoNo"].ToString().Trim();
            CoName1.Text = reader["CoName1"].ToString().Trim();
        ...
        private void InNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (jSaleDis.IsInNoFormat(sender, e) == false)
                return;
            ...
            {
                count = db.Count("SaleInv", "inno", InNo.Text.Trim());
            }
            if (count == 0)
            {
                ...
                {
                    jSaleDis.ValidateOpen<JBS.JS.SaleInv>(sender, e, reader =>
                    {
            ...
            //是否作廢
            if (jSaleDis.IsInvalid(InNo.Text))
            {
                e.Cancel = true;
            ...
            //if (jSaleDis.IsExistDocument<JBS.JS.SaleDis>(InNo.Text.Trim()) == true)
            //{
            //    e.Cancel = true;
        ...
        private void DiNo_Validating(object sender, CancelEventArgs e)
        {
