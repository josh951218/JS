...
{
    public partial class FrmOrderSaleda : Formbase
    {
        ...
        public FrmOrderSaleda()
        {
            ...
            this.售價.Visible = this.稅前售價.Visible = this.稅前金額.Visible = Common.User_SalePrice;
        }

        private void FrmOrderSaleda_Load(object sender, EventArgs e)
        {
            //產品類別轉為文字
        ...
        void WriteToTitle(IGrouping<string, DataRow> grow)
        {
            if (grow.IsNotNull())
            {
                ...
                textBoxT3.Text = temp.Rows[0]["cuno"].ToString().Trim();
                textBoxT4.Text = temp.Rows[0]["cuname1"].ToString().Trim();
        ...
        private void btnBrowT1_Click(object sender, EventArgs e)
        {
            ...
            temp.DefaultView.Sort = "itno,itunit,sadate,sano";
            SetButtonColor();
        ...
        private void btnBrowT2_Click(object sender, EventArgs e)
        {
            txtmiddle = "訂單日期別";
            NO = dataGridViewT1.SelectedRows[0].Cells["序號"].Value.ToString();
            temp.DefaultView.Sort = "sadate,sano";
            SetButtonColor();
        ...
        void paramsInit()
        {
            ...
            if (Common.Sys_DBqty == 1)
                path = Common.reportaddress + "Report\\" + txtmiddle + HaveO + HaveBom + "_銷貨明細表.rpt";
            else if (Common.Sys_DBqty == 2)
                path = Common.reportaddress + "Report\\" + txtmiddle + HaveO + HaveBom + "_銷貨明細表P.rpt";
            ...
            rp = new RPT(DtD, path);
            rp.office = txtmiddle + "銷貨明細表";
            rp.lobj.Add(new string[] { "txtend", txtend });
            //rp.lobj.Add(new string[] { "txtmiddle", txtmiddle + "-銷貨明細表" });
        ...
        private void FrmOrderSaleda_Shown(object sender, EventArgs e)
        {
            btnPreView.Focus();
        }
