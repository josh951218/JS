...
{
    public partial class FrmItemNotOut : Formbase
    {
        JBS.JS.xEvents xe;
        ...
        public FrmItemNotOut()
        {
            InitializeComponent();
            this.xe = new JBS.JS.xEvents();
        }

        private void FrmItemNotOut_Load(object sender, EventArgs e)
        {
        ...
        private void btnBrow_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlConnection cn = new SqlConnection(Common.sqlConnString))
                {
                    //不含組件
                    cn.Open();
                    sql = "SELECT odd.pqty,odd.punit,odd.esdate,odd.esdate1,odd.orno,odd.itno,odd.itname,odd.qty,odd.itunit,"
                    ...
                    if (EsDate.Text.ToString().Trim().Length > 0) sql += " and odd.esdate >=@date";
                    if (EsDate1.Text.ToString().Trim().Length > 0) sql += " and odd.esdate <=@date1";
                    if (ItNo.Text.ToString().Trim().Length > 0) sql += " and odd.itno >=@itno";
                    if (ItNo1.Text.ToString().Trim().Length > 0) sql += " and odd.itno <=@itno1";
                ...
                if (Dt.Rows.Count > 0)
                {
                    //using (var frm = new FrmItemNotOutb())
                    //{
                    //    frm.Dt = Dt.Copy();
                    //    frm.DtD = DtD.Copy();
                    ...
                    //    frm.ShowDialog();
                    //}
                    this.OpemInfoFrom<FrmItemNotOutb>(() =>
                            {
                                FrmItemNotOutb frm = new FrmItemNotOutb();
                                frm.Dt = Dt.Copy();
                                frm.DtD = DtD.Copy();
                                frm.DateRange = DateRange;
                                return frm;
