...
{
    public partial class FrmCustNotOut : Formbase
    {
        JBS.JS.xEvents xe;
        ...
        public FrmCustNotOut()
        {
            InitializeComponent();
            this.xe = new JBS.JS.xEvents();
        }

        private void FrmCustNotOut_Load(object sender, EventArgs e)
        {
        ...
        private void CuNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                tb.Text = reader["cuno"].ToString().Trim();
            }, true); 
        }
            ...
            try
            {
                ...
                using (SqlConnection cn = new SqlConnection(Common.sqlConnString))
                {
                    ...
                    sql = "SELECT odd.esdate,odd.esdate1,odd.orno,odd.itno,odd.itname,odd.qty,odd.itunit,odd.pqty,odd.punit,"
                        ...
                        + "WHERE od.oroverflag <> 1 and odd.qtyNotOut>0 ";
                    ...
                    if (CuNo.Text.ToString().Trim().Length > 0) sql += " and odd.cuno >=@cuno ";
                    if (CuNo1.Text.ToString().Trim().Length > 0) sql += " and odd.cuno <=@cuno1 ";
                    ...
                    if (CuNo.Text.ToString().Trim().Length > 0) cmd.Parameters.AddWithValue("cuno", CuNo.Text.ToString().Trim());
                    if (CuNo1.Text.ToString().Trim().Length > 0) cmd.Parameters.AddWithValue("cuno1", CuNo1.Text.ToString().Trim());
                    cmd.CommandText = sql;
                    SqlDataAdapter dd;
                ...
                if (Dt.Rows.Count > 0)
                {
                    //using (FrmCustNotOutb frm = new FrmCustNotOutb())
                    //{ 
                    //    frm.Dt = Dt.Copy();
                    //    frm.DtD = DtD.Copy();
                    ...
                    //    frm.ShowDialog();
                    //}
                    this.OpemInfoFrom<FrmCustNotOutb>(() =>
                            {
                                FrmCustNotOutb frm = new FrmCustNotOutb();
                                frm.Dt = Dt.Copy();
                                frm.DtD = DtD.Copy();
                                frm.DateRange = DateRange;
                                return frm;
        ...
        private void FrmCustNotOut_Shown(object sender, EventArgs e)
        {
            EsDate.Focus();
        }
