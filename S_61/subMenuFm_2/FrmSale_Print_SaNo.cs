...
{
    public partial class FrmSale_Print_SaNo : Formbase, JBS.JS.IxOpen
    { 
        ...
        public FrmSale_Print_SaNo()
        {
            InitializeComponent();

            this.銷貨總額.DefaultCellStyle.Format = "f" + Common.MST;
            ...
            this.銷貨總額.Visible = Common.User_SalePrice;
            this.單價.Visible = Common.User_SalePrice;
        }

        private void FrmSale_Print_SaNo_Load(object sender, EventArgs e)
        {
            this.TResult = "";
            ...
            if (Common.User_DateTime == 1)
                this.銷貨日期.DataPropertyName = "SaDate";
            else
                this.銷貨日期.DataPropertyName = "SaDate1";
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Common.sqlConnString))
                {
                    string sql = "select x3name = "
                        ...
                        + " when x3no=3 then '零稅'"
                        + " when x3no=4 then '免稅'"
                        + " end,* from Sale order by SaNo COLLATE Chinese_Taiwan_Stroke_BIN";
        ...
        private void btnGet_Click(object sender, EventArgs e)
        {
            ...
            {
                var TempID = dataGridViewT1.SelectedRows[0].Cells["銷貨單號"].Value.ToString();
        ...
        private void dataGridViewT1_RowStateChanged(object sender, DataGridViewRowStateChangedEventArgs e)
        {
            ...
            if (e.StateChanged == DataGridViewElementStates.Selected)
            {
                try
                {
                    string sano = dataGridViewT1.SelectedRows[0].Cells["銷貨單號"].Value.ToString();
                    using (SqlConnection conn = new SqlConnection(Common.sqlConnString))
                    {
                        string sql = "select * from saled where sano=@sano COLLATE Chinese_Taiwan_Stroke_BIN";
                        using (SqlCommand cmd = conn.CreateCommand())
                        {
                            cmd.Parameters.Clear();
