...
{
    public partial class FrmRShop_Print_BsNo : Formbase
    {
        ...
        private void FrmRShop_Print_BsNo_Load(object sender, EventArgs e)
        {
            this.TResult = "";
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Common.sqlConnString))
                {
                    string sql = "select x3name = "
                        ...
                        + " when x3no=3 then '零稅'"
                        + " when x3no=4 then '免稅'"
                        + " end,* from RShop order by BsNo COLLATE Chinese_Taiwan_Stroke_BIN";
        ...
        private void dataGridViewT1_RowStateChanged(object sender, DataGridViewRowStateChangedEventArgs e)
        {
            ...
            if (e.StateChanged == DataGridViewElementStates.Selected)
            {
                try
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(Common.sqlConnString))
                    {
                        string sql = "select * from RShopD where BsNo=@bsno COLLATE Chinese_Taiwan_Stroke_BIN";
                        using (SqlCommand cmd = conn.CreateCommand())
                        {
                            cmd.Parameters.Clear();
