...
{
    public partial class FrmGarner_Infob : Formbase
    {
        ...
        private void FrmGarner_Infob_Load(object sender, EventArgs e)
        {
            ReportFileName = "入庫資料瀏覽";
             
            rd2.SetUserDefineRpt("入庫資料瀏覽_簡要自訂.rpt");
            rd3.SetUserDefineRpt("入庫資料瀏覽_組件自訂.rpt");
             
            this.入庫數量.DefaultCellStyle.Format = "f" + Common.Q;
            this.包裝數量.DefaultCellStyle.Format = "f" + Common.Q;
            this.成本.DefaultCellStyle.Format = "f" + Common.M;
            ...
            try
            {
                ...
                if (rd5.Checked)
                {
                    using (SqlConnection cn = new SqlConnection(Common.sqlConnString))
                    {
                        ...
                        sql += " left join garnbom as C on C.bomid = B.bomid where '0'='0'";
                        if (stnoi != "")
                        ...
                        using (SqlCommand cmd = cn.CreateCommand())
                        {
                            ...
                            if (stnoo1 != "") cmd.Parameters.AddWithValue("stnoo",stnoo1);
                            ...
                            cmd.Parameters.AddWithValue("gadate",date);
                            cmd.Parameters.AddWithValue("gadate1",date1);
                            cmd.CommandText=sql;
                            using (SqlDataAdapter dd = new SqlDataAdapter(cmd))
                            {
                                ...
                                {
                                    if (Common.User_DateTime == 1)
                                        tableB.Rows[i]["gadate"] = Date.AddLine(tableB.Rows[i]["gadate"].ToString());
                                    else
                                        tableB.Rows[i]["gadate1"] = Date.AddLine(tableB.Rows[i]["gadate1"].ToString());
                                }
        ...
        private void qury2_Click(object sender, EventArgs e)
        {
            ...
            table.DefaultView.Sort = "gadate,gano";
            SetButtonColor();
            qury2.ForeColor = Color.Red;
            SetSelectRow(NO);
        }
        ...
        private void qury3_Click(object sender, EventArgs e)
        {
            SortState = "F3";
            NO = dataGridViewT1.SelectedRows[0].Cells["序號"].Value.ToString().Trim();
            table.DefaultView.Sort = "itno,gadate";
            SetButtonColor();
        ...
        private void qury4_Click(object sender, EventArgs e)
        {
            SortState = "F4";
            NO = dataGridViewT1.SelectedRows[0].Cells["序號"].Value.ToString().Trim();
            table.DefaultView.Sort = "emno,gadate";
            SetButtonColor();
        ...
        private void qury5_Click(object sender, EventArgs e)
        {
            SortState = "F5";
            NO = dataGridViewT1.SelectedRows[0].Cells["序號"].Value.ToString().Trim();
            table.DefaultView.Sort = "memo,gadate";
            SetButtonColor();
