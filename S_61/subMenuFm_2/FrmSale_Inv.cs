...
{
    public partial class FrmSale_Inv : Formbase
    {
        ...
        public FrmSale_Inv(string name, string pk, bool b)
        {
            InitializeComponent();
            Result = new string[4];
        ...
        private void FrmSale_Inv_Load(object sender, EventArgs e)
        {
        ...
        {
            //媒體申報
            if (InvKind.Text.Substring(0, 2) == "37")
            {
                SpecialTax.Enabled = true;

                if (SpecialTax.Text == "")
                    SpecialTax.SelectedIndex = 0;
            }
            else
            {
                SpecialTax.Enabled = false;
            }   
        }
    }
}
