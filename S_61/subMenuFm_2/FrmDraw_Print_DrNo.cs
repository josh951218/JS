...
{
    public partial class FrmDraw_Print_DrNo : Formbase
    {
        ...
        public FrmDraw_Print_DrNo()
        {
            InitializeComponent();
            this.數量.DefaultCellStyle.Format = "f" + Common.Q;
        }

        private void FrmDraw_Print_DrNo_Load(object sender, EventArgs e)
        {
            this.TResult = "";

            if (Common.User_DateTime == 1)
                this.領料日期.DataPropertyName = "drdate1";
            else
                this.領料日期.DataPropertyName = "drdate2";


            loadM();
            if (list.Count > 0)
            {
                index = list.FindLastIndex(r => string.CompareOrdinal(this.TSeekNo, r["drno"].ToString()) == 0);
                if (index == -1)
                {
                    index = list.FindLastIndex(r => string.CompareOrdinal(this.TSeekNo, r["drno"].ToString()) > 0);
        ...
        void loadM()
        {
            try
            {
                ...
                {
                    cn.Open();
                    SqlDataAdapter dd = new SqlDataAdapter("select * from draw order by drno", cn);
                    Master.Clear();
                    list.Clear();
                    dd.Fill(Master);
        ...
        private void dataGridViewT1_SelectionChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    cn.Open();
                    string sql = "select 序號='',* from drawd where drno=@drno";
                    using (SqlCommand cmd = cn.CreateCommand())
                    {
                        cmd.Parameters.Clear();
                        cmd.Parameters.AddWithValue("drno", dataGridViewT1.SelectedRows[0].Cells["領料單號"].Value.ToString());
                        cmd.CommandText = sql;
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection cn = new SqlConnection(Common.sqlConnString))
                {
                    string sql = "select * from draw where drno like '%' + @drno + '%' order by drno";
                    using (SqlCommand cmd = cn.CreateCommand())
                    {
        ...
        {
            if (dataGridViewT1.SelectedRows.Count == 0) return;
            string drno = dataGridViewT1.SelectedRows[0].Cells["領料單號"].Value.ToString();
        ...
        private void DrNo_TextChanged(object sender, EventArgs e)
        {
            if (DrNo.Text == "") return;
            loadM();
            if (list.Count > 0)
            {
                index = list.FindIndex(r => string.CompareOrdinal(DrNo.Text, r["drno"].ToString()) == 0);
                if (index == -1)
                {
                    index = list.FindLastIndex(r => string.CompareOrdinal(DrNo.Text, r["drno"].ToString()) > 0);
                    if (index == -1 || ++index >= list.Count)
                        index = list.Count - 1;
                    dataGridViewT1.FirstDisplayedScrollingRowIndex = index;
                    dataGridViewT1.Rows[index].Selected = true;
