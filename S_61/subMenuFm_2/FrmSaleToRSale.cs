...
{
    public partial class FrmSaleToRSale : Formbase
    {
        public string SaNo = "";
        public string CuNo = "";
        ...
        public FrmSaleToRSale()
        {
            InitializeComponent();
            cn.ConnectionString = Common.sqlConnString;

            this.銷售總額.DefaultCellStyle.Format = "f" + Common.MST;
        ...
        void LoadMaster()
        {
            ...
            if (CuNo.Trim() == "")
                daM.SelectCommand.CommandText = @"
                    ...
                    when x3no=3 then '零稅'
                    when x3no=4 then '免稅'
                    end,* from sale order by sano desc";
            daM.Fill(dtM);
        }
        ...
        void LoadDetail(object sano)
        {
            ...
            daD.SelectCommand.CommandText = @"select *,ItNoUdf = (select  top  1  ItNoUdf  from item where item.itno = saled.itno )  
                                                from saled where sano=@No order by recordno";
            ...
            if (dataGridViewT1.Rows.Count > 0)
                if (dataGridViewT1.SelectedRows != null)
                    if (dataGridViewT1.SelectedRows.Count > 0)
                        ...
                        {
                            cn.Close();
                            LoadDetail(dataGridViewT1["銷貨單號", e.Row.Index].Value);
                        }
        }
                ...
                if (dataGridViewT1.Rows.Count > 0)
                    if (dataGridViewT1.SelectedRows != null)
                        if (dataGridViewT1.SelectedRows.Count > 0)
                        {
                            ...
                            SaNo = dataGridViewT1.SelectedRows[0].Cells["銷貨單號"].Value.ToString();
                        }
            }
        ...
        {
            if (dataGridViewT1.Rows.Count > 0)
                dataGridViewT1.Search("銷貨單號", qNo.Text);
        }
    }
}
