...
{
    public partial class FrmOrderToSale : Formbase
    {
        ...
        private void FrmOderToSale_Load(object sender, EventArgs e)
        {
        ...
        {
            base.OnShown(e);
            送貨地址.Width = (int)((13 * JEInitialize.CharWidth) + 7 + (double)JEInitialize.CharWidth * ((double)14 / (double)18));
        ...
        void LoadDetail(object orno)
        {
            ...
            daD.SelectCommand.CommandText = @"SELECT *,ItNoUdf = (select  top  1  ItNoUdf  from item where item.itno = orderd.itno )   
                                              FROM orderd WHERE orno = @NO order by recordno";
        ...
        private void dataGridViewT2_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //如果訂單數量 > 0 的話，但未交量為 0 時，不得取回；而如果訂單數量一開始就是 0 ，不管未交量是否沒0皆可以取回  開會討論通過(秀英提出) 日期 : 20160127 
            if (dataGridViewT2["數量", e.RowIndex].Value.ToDecimal() > 0 && dataGridViewT2["未交量", e.RowIndex].Value.ToDecimal() == 0) return;
        ...
        {
            if (dataGridViewT1.Rows.Count > 0)
                dtM.DefaultView.Search(ref dataGridViewT1, "OrNo", qNo.Text);
        }
    }
}
