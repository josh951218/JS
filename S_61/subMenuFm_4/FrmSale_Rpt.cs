...
{
    public partial class FrmSale_Rpt : Formbase
    {
        class 成本計算
        {
            bool sale = false;
            ...
            public void 月平均成本(out DataTable dtResult)
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    ...
                    if (sale)
                    {
                        cmd.CommandText = "Select substring(sadate,1,5)月份,cuno,itno,itname,stno,銷退金額=0.0,銷貨金額=mnyb,銷退數量=0.0,銷貨數量=qty*itpkgqty,母數量=qty*itpkgqty from saled where ittrait <> 1 " + sQuery;
                        da.Fill(saled23);
                    }

                    if (rsale)
                    {
                        cmd.CommandText = "Select substring(sadate,1,5)月份,cuno,itno,itname,stno,銷退金額=(-1)*mnyb,銷貨金額=0.0,銷貨數量=0.0,銷退數量=(-1)*qty*itpkgqty,母數量=(-1)*qty*itpkgqty from rsaled where ittrait <> 1 " + rQuery;
                        da.Fill(rsaled23);
                    }
                    ...
                    {
                        cmd.CommandText = @"
                        Select 銷貨成本=0.0,saled.bomid,substring(sadate,1,5)月份,saled.cuno,saled.itno sitno,saled.itname,saled.stno,bom.itno,銷退金額=0.0,銷貨金額=saled.mnyb,銷退數量=0.0,銷貨數量=saled.qty*saled.itpkgqty,母數量=saled.qty*saled.itpkgqty,子數量=(bom.itqty*bom.itpkgqty/bom.itpareprs) from salebom bom
                    ...
                    {
                        cmd.CommandText = @"
                        Select 銷貨成本=0.0,rsaled.bomid,substring(sadate,1,5)月份,rsaled.cuno,rsaled.itno sitno,rsaled.itname,rsaled.stno,bom.itno,銷退金額=(-1)*rsaled.mnyb,銷貨金額=0.0,銷貨數量=0.0,銷退數量=(-1)*rsaled.qty*rsaled.itpkgqty,母數量=(-1)*rsaled.qty*rsaled.itpkgqty,子數量=(bom.itqty*bom.itpkgqty/bom.itpareprs) from rsalebom bom
            ...
            void AvgCost(DataTable tSale, DataTable itemCost, ref DataTable dtSource)
            {
                ...
                var temp = tSale.AsEnumerable()
                    .GroupJoin(
                        ...
                        s => new { ym = s["月份"].ToString(), itno = s["itno"].ToString() },//tSale
                        c => new { ym = c["月份"].ToString(), itno = c["itno"].ToString() },//itemCost
            ...
            public void 最後一次進貨成本(out DataTable dtResult)
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    ...
                    {
                        cmd.CommandText = @"
                        Select saled.sadate,saled.cuno,saled.itno,saled.itname,銷退金額=0.0,銷貨金額=saled.mnyb,銷退數量=0.0,銷貨數量=saled.qty*saled.itpkgqty,母數量=saled.qty,saled.itunit,saled.itpkgqty,item.itbuypri
                        From saled 
                    ...
                    {
                        cmd.CommandText = @"
                        Select rsaled.sadate,rsaled.cuno,rsaled.itno,rsaled.itname,銷退金額=(-1)*rsaled.mnyb,銷貨金額=0.0,銷貨數量=0.0,銷退數量=(-1)*rsaled.qty*rsaled.itpkgqty,母數量=(-1)*rsaled.qty,rsaled.itunit,rsaled.itpkgqty,item.itbuypri
                        From rsaled 
                    ...
                    {
                        cmd.CommandText = @"
                        Select 銷貨成本=0.0,saled.bomid,saled.sadate,saled.cuno,saled.itno sitno,saled.itname,bom.itno,銷退金額=0.0,銷貨金額=saled.mnyb,銷退數量=0.0,銷貨數量=saled.qty*saled.itpkgqty,母數量=saled.qty*saled.itpkgqty,子數量=(bom.itqty/bom.itpareprs),bom.itpkgqty,bom.itunit,item.itbuypri
                        From saled 
                        Left join item on saled.itno = item.itno
                    ...
                    {
                        cmd.CommandText = @"
                        Select 銷貨成本=0.0,rsaled.bomid,rsaled.sadate,rsaled.cuno,rsaled.itno sitno,rsaled.itname,bom.itno,銷退金額=(-1)*rsaled.mnyb,銷貨金額=0.0,銷貨數量=0.0,銷退數量=(-1)*rsaled.qty*rsaled.itpkgqty,母數量=(-1)*rsaled.qty*rsaled.itpkgqty,子數量=(bom.itqty/bom.itpareprs),bom.itpkgqty,bom.itunit,item.itbuypri
                        From rsaled 
                        Left join item on rsaled.itno = item.itno
            ...
            public void 標準成本(out DataTable dtResult)
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    ...
                    if (sale)
                    {
                        cmd.CommandText = "Select cuno,itno,itname,銷退金額=0.0,銷貨金額=mnyb,銷退數量=0.0,銷貨數量=qty*itpkgqty,母數量=qty,itpkgqty from saled where ittrait <> 1 " + sQuery;
                        da.Fill(saled23);
                    }

                    if (rsale)
                    {
                        cmd.CommandText = "Select cuno,itno,itname,銷退金額=(-1)*mnyb,銷貨金額=0.0,銷貨數量=0.0,銷退數量=(-1)*qty*itpkgqty,母數量=(-1)*qty,itpkgqty from rsaled where ittrait <> 1 " + rQuery;
                        da.Fill(rsaled23);
                    }
                    ...
                    {
                        cmd.CommandText = @"
                        Select 銷貨成本=0.0,saled.bomid,saled.cuno,saled.itno sitno,saled.itname,bom.itno,銷退金額=0.0,銷貨金額=saled.mnyb,銷退數量=0.0,銷貨數量=saled.qty*saled.itpkgqty,母數量=saled.qty*saled.itpkgqty,子數量=(bom.itqty/bom.itpareprs),bom.itpkgqty from salebom bom
                        right join saled on bom.bomid = saled.bomid 
                        where saled.ittrait = 1 and (bom.itpareprs <> 0 or bom.itpareprs is null) " + sQuery;
                        da.Fill(saled1);
                    }
                    ...
                    {
                        cmd.CommandText = @"
                        Select 銷貨成本=0.0,rsaled.bomid,rsaled.cuno,rsaled.itno sitno,rsaled.itname,bom.itno,銷退金額=(-1)*rsaled.mnyb,銷貨金額=0.0,銷貨數量=0.0,銷退數量=(-1)*rsaled.qty*rsaled.itpkgqty,母數量=(-1)*rsaled.qty*rsaled.itpkgqty,子數量=(bom.itqty/bom.itpareprs),bom.itpkgqty from rsalebom bom
                        right join rsaled on bom.bomid = rsaled.bomid 
                        where rsaled.ittrait = 1 and (bom.itpareprs <> 0 or bom.itpareprs is null) " + rQuery;
                        da.Fill(rsaled1);
                    }
        ...
        private void FrmSale_Rpt_Load(object sender, EventArgs e)
        {
        ...
        private void FrmSale_Rpt_Shown(object sender, EventArgs e)
        {
            SaDate.Focus();
        }
        ...
        private void btnBrow_Click(object sender, EventArgs e)
        {
            ...
            {
                new 成本計算(ckSale.Checked, ckRsale.Checked, SaDate.Text, SaDate1.Text, CuNo.Text, CuNo1.Text,false).月平均成本(out dtResult);
            }
            else if (rdAvgByOneStk.Checked)
            {
                new 成本計算(ckSale.Checked, ckRsale.Checked, SaDate.Text, SaDate1.Text, CuNo.Text, CuNo1.Text, true).月平均成本(out dtResult);
            }
            ...
            {
                #region rdSimple
                //using (var frm = new FrmSale_Rptb())
                //{
                //    frm.dt = dtResult;
                ...
                //    dtResult.Clear();
                //}
                this.OpemInfoFrom<FrmSale_Rptb>(() =>
                            {
                                FrmSale_Rptb frm = new FrmSale_Rptb();
            ...
            else
            {
                ...
                //using (var frm = new FrmSale_Rptc())
                //{
                ...
                //    dtResult.Clear();
                //}
                this.OpemInfoFrom<FrmSale_Rptc>(() =>
                            {
                                FrmSale_Rptc frm = new FrmSale_Rptc();
                                frm.dt = tTemp.AsEnumerable().OrderBy(r => r["cuno"].ToString()).CopyToDataTable();
                                frm.dateRange = SaDate.Text + "～" + SaDate1.Text;
                                return frm;
                            });
