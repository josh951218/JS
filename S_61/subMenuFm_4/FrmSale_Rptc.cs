...
{
    public partial class FrmSale_Rptc : Formbase
    {
        ...
        public FrmSale_Rptc(bool isFastReport = false)
        {
            InitializeComponent();
            ...
            this.毛利率.DefaultCellStyle.Format = "f2";
        ...
        private void FrmSale_Rptc_Load(object sender, EventArgs e)
        {
            if (this.IsFastReport)
            {
                rd2.SetUserDefineRpt("Recustud3.frx", @"ReportF\");
                rd3.SetUserDefineRpt("Recustud4.frx", @"ReportF\");
            }
            else
            {
        ...
        void SetSortBtn()
        {
            if (dataGridViewT1.SelectedRows.Count > 0)
                CuNoSelected = dataGridViewT1["客戶編號", dataGridViewT1.SelectedRows[0].Index].Value.ToString();
        }
        ...
        {
            if (dt.Rows.Count == 0 && mode != RptMode.Design)
            {
                MessageBox.Show("查無資料!");
                return;
            }

            var path = Common.reportaddress + @"ReportF\會員銷售排行表.frx";
