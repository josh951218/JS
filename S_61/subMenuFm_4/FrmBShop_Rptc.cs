...
{
    public partial class FrmBShop_Rptc : Formbase
    { 
        ...
        public FrmBShop_Rptc()
        {
            InitializeComponent();

            ReportFileName = "廠商進貨報表_總額表.rpt"; 
            ...
            txtNumber1.Visible = txtNumber2.Visible = txtNumber3.Visible = Common.User_ShopPrice;
        }

        private void FrmBShop_Rptc_Load(object sender, EventArgs e)
        {
            dataGridViewT1.DataSource = dtResult;
            ...
            {
                退貨金額 += dtResult.Rows[i]["退貨金額"].ToDecimal();
                進貨金額 += dtResult.Rows[i]["進貨金額"].ToDecimal();
                進貨淨額 += dtResult.Rows[i]["進貨淨額"].ToDecimal();
            }
            txtNumber1.Text = 退貨金額.ToString("f" + Common.TPF);
            txtNumber2.Text = 進貨金額.ToString("f" + Common.TPF);
            txtNumber3.Text = 進貨淨額.ToString("f" + Common.TPF);
        }
