...
{
    public partial class 帳款匯入作業 : Formbase
    {
        ...
        public 帳款匯入作業()
        {
            ...
            Txt = new List<TextBox> { ReNo, CoNo, CoName1, CoName2, ReDate, AcNo, AcName, AcName1, Xa1Name, ReMny, ReMemo, CuNo, CuName1, CuName2 };

            ReMny.MarkThousand = true;
        ...
        {
            ReMny.LastNum = Convert.ToInt32(chksys["deci"].ToString());
            ReMny.FirstNum = 12;
            ReMny.MaxLength = 20;
        ...
        private void btnExit_Click(object sender, EventArgs e)
        {
            if (!開窗模式)
            {
                ...
                pVar.FrmReceivd.RemitMny.Text = tot.ToString("f" + pVar.FrmReceivd.RemitMny.LastNum);
                pVar.FrmReceivd.RemitI = tbM.Copy();
            }
            this.Dispose();
            this.Close();
        }
            ...
            try
            {
                ...
                else
                {
                    using (SqlConnection cn = new SqlConnection(票據連線字串))
                    {
                        ...
                        cmd.CommandText = "select reno from remiti where reno=@reno";
