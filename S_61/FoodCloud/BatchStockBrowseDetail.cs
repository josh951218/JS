...
{
    public partial class BatchStockBrowseDetail : Formbase
    {
        public DataTable dt = new DataTable();
        DataTable orderdt = new DataTable();
        string NO = "";
        List<Button> qury;
        public BatchStockBrowseDetail()
        {
            InitializeComponent();
        ...
        private void dataintodocument(RptMode rptMode)
        {
            using (JBS.FastReport_Wei FastReport = new JBS.FastReport_Wei())
            {
                ...
                else 
                {
                    if (orderbyitno.ForeColor == Color.Red)
                    {
                        報表名稱 = "批號現有庫存明細表_產品排序_自定一";
                    }
                    else
                    {
                        報表名稱 = "批號現有庫存明細表_批次排序_自定一";
                    }
                }
        ...
        void SearchUserReport()
        {
            ...
            if (orderbyitno.ForeColor == Color.Red)
                radioT1.SetUserDefineRpt("批號現有庫存明細表_產品排序_自定一.frx",@"ReportG\");
