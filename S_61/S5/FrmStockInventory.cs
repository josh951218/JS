...
namespace S_61.S5
{
    public partial class FrmStockInventory : Formbase
    {
        ...
        public FrmStockInventory()
        {
            ...
            radioT3.SetUserDefineRpt("產品現有庫存表_明細表_自定一.rpt");
            radioT4.SetUserDefineRpt("產品現有庫存表_明細表_自定二.rpt");
        }
        ...
        void OutReport(RptMode mode)
        {
            ...
            if (radioT1.Checked) path += "產品現有庫存表_明細表_標準報表.rpt";
            if (radioT2.Checked) path += "產品現有庫存表_明細表_雙單位表.rpt";
            if (radioT3.Checked) path += "產品現有庫存表_明細表_自定一.rpt";
            if (radioT4.Checked) path += "產品現有庫存表_明細表_自定二.rpt";

            RPT rp = new RPT(dt, path);

            var txtend = "";
            if (radioT5.Checked) txtend = Common.dtEnd.Rows[0]["tamemo"].ToString();
            else if (radioT6.Checked) txtend = Common.dtEnd.Rows[1]["tamemo"].ToString();
            else if (radioT7.Checked) txtend = Common.dtEnd.Rows[2]["tamemo"].ToString();
            else if (radioT8.Checked) txtend = Common.dtEnd.Rows[3]["tamemo"].ToString();
            else if (radioT9.Checked) txtend = Common.dtEnd.Rows[4]["tamemo"].ToString();
            else txtend = "";

            rp.lobj.Add(new string[] { "CreateDate", Date.GetDateTime(Common.User_DateTime, true) });
            rp.lobj.Add(new string[] { "txtend", txtend });

            if ((radioT1.Checked || radioT2.Checked) && mode == RptMode.Excel)
            {
