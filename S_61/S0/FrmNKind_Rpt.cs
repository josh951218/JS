...
namespace S_61.S0
{
    public partial class FrmNKind_Rpt : Formbase
    {
        class 成本計算
        {
            bool ifkind = false;
            ...
            public 成本計算(bool s, bool r, Action<SqlParameterCollection> spara, string sqstr, string rqstr, bool ifkind, bool ifxx01)
            {
                this.sale = s;
                this.rsale = r;
            ...
            public void 月平均成本(out DataTable dtResult)
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    ...
                    var back = FrmNKind_Rpt.BackEnd ? "" : " And saled.bracket='前台' ";
                    var rback = FrmNKind_Rpt.BackEnd ? "" : " And rsaled.bracket='前台' ";
                    ...
                    ,產品編號=saled.itno,品名規格=saled.itname,數量=saled.qty,單位=saled.itunit,售價=saled.price,折數=saled.prs,稅前售價=saled.taxprice,稅前金額=(" + mny + @"),包裝數量=saled.itpkgqty,說明1=saled.memo
                    ...
                    ,產品編號=rsaled.itno,品名規格=rsaled.itname,數量=(-1)*rsaled.qty,單位=rsaled.itunit,售價=rsaled.price,折數=rsaled.prs,稅前售價=rsaled.taxprice,稅前金額=(-1)*(" + rmny + @"),包裝數量=rsaled.itpkgqty,說明1=rsaled.memo
                    ,業務人員=SA.emname,送貨方式=SA.sename,專案名稱=SA.spname,結帳方式=SA.x4name,說明=SA.samemo
                    ,客戶簡稱=cust.cuname1,客戶名稱=cust.cuper1,客戶電話=cust.cutel1,客戶傳真=cust.cufax1,客戶類別=cust.cux1no
                    ,客戶類別名稱=X1.x1name
                    ,輔助編號=item.itnoudf,產品類別=item.kino
                    ...
                    {
                        cmd.CommandText = @"
                        Select 單據='銷貨',saled.bomid,substring(sadate,1,5)月份,saled.itno,mny=ISNULL((" + mny + @"),0),母數量=saled.qty*saled.itpkgqty from saled " + sxx01 + skind + @"
                    ...
                    {
                        cmd.CommandText = @"
                        Select 單據='銷退',rsaled.bomid,substring(sadate,1,5)月份,rsaled.itno,mny=(-1)*ISNULL((" + rmny + @"),0),母數量=(-1)*rsaled.qty*rsaled.itpkgqty from rsaled " + rxx01 + rkind + @"
                    ...
                    {
                        cmd.CommandText = @"
                        Select 成本=0.0,單據='銷貨',saled.bomid,substring(sadate,1,5)月份,saled.itno sitno,bom.itno,mny=ISNULL((" + mny + @"),0),母數量=saled.qty*saled.itpkgqty,子數量=(bom.itqty*bom.itpkgqty/bom.itpareprs) from saled " + sxx01 + skind + @"
                    ...
                    {
                        cmd.CommandText = @"
                        Select 成本=0.0,單據='銷退',rsaled.bomid,substring(sadate,1,5)月份,rsaled.itno sitno,bom.itno,mny=(-1)*ISNULL((" + rmny + @"),0),母數量=(-1)*rsaled.qty*rsaled.itpkgqty,子數量=(bom.itqty*bom.itpkgqty/bom.itpareprs) from rsaled " + rxx01 + rkind + @"
            ...
            public void 最後一次進貨成本(out DataTable dtResult)
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    ...
                    {
                        cmd.CommandText = @"
                        Select 成本=0.0,單據='銷貨',saled.bomid,saled.sadate,saled.itno sitno,bom.itno,mny=ISNULL((" + mny + @"),0),母數量=saled.qty*saled.itpkgqty,子數量=(bom.itqty/bom.itpareprs),bom.itpkgqty,bom.itunit,item.itbuypri
                        From saled " + sxx01 + skind + @" 
                        Left join salebom bom on bom.bomid = saled.bomid 
                        Where saled.ittrait = 1 and (bom.itpareprs <> 0 or bom.itpareprs is null) " + back + sQuery;
                        da.Fill(saled1);
                    ...
                    {
                        cmd.CommandText = @"
                        Select 成本=0.0,單據='銷退',rsaled.bomid,rsaled.sadate,rsaled.itno sitno,bom.itno,mny=(-1)*ISNULL((" + rmny + @"),0),母數量=(-1)*rsaled.qty*rsaled.itpkgqty,子數量=(bom.itqty/bom.itpareprs),bom.itpkgqty,bom.itunit,item.itbuypri
                        From rsaled " + rxx01 + rkind + @" 
                        Left join rsalebom bom on bom.bomid = rsaled.bomid 
                        Where rsaled.ittrait = 1 and (bom.itpareprs <> 0 or bom.itpareprs is null) " + rback + rQuery;
                        da.Fill(rsaled1);
            ...
            public void 標準成本(out DataTable dtResult)
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    ...
                    {
                        cmd.CommandText = @"
                        Select 單據='銷貨',saled.bomid,saled.itno,mny=ISNULL((" + mny + @"),0),母數量=saled.qty,saled.itpkgqty from saled " + sxx01 + skind + @"
                        where saled.ittrait <> 1 " + back + sQuery;
                        da.Fill(saled23);
                    }
                    ...
                    {
                        cmd.CommandText = @"
                        Select 單據='銷退',rsaled.bomid,rsaled.itno,mny=(-1)*ISNULL((" + rmny + @"),0),母數量=(-1)*rsaled.qty,rsaled.itpkgqty from rsaled " + rxx01 + rkind + @"
                        where rsaled.ittrait <> 1 " + rback + rQuery;
                        da.Fill(rsaled23);
                    }
                    ...
                    {
                        cmd.CommandText = @"
                        Select 成本=0.0,單據='銷貨',saled.bomid,saled.itno sitno,bom.itno,mny=ISNULL((" + mny + @"),0),母數量=saled.qty*saled.itpkgqty,子數量=(bom.itqty/bom.itpareprs),bom.itpkgqty from saled  " + sxx01 + skind + @"
                        left join salebom bom on bom.bomid = saled.bomid 
                        where saled.ittrait = 1 and (bom.itpareprs <> 0 or bom.itpareprs is null) " + back + sQuery;
                        da.Fill(saled1);
                    }
                    ...
                    {
                        cmd.CommandText = @"
                        Select 成本=0.0,單據='銷退',rsaled.bomid,rsaled.itno sitno,bom.itno,mny=(-1)*ISNULL((" + rmny + @"),0),母數量=(-1)*rsaled.qty*rsaled.itpkgqty,子數量=(bom.itqty/bom.itpareprs),bom.itpkgqty from rsaled " + rxx01 + rkind + @"
                        left join rsalebom bom on bom.bomid = rsaled.bomid 
                        where rsaled.ittrait = 1 and (bom.itpareprs <> 0 or bom.itpareprs is null) " + rback + rQuery;
                        da.Fill(rsaled1);
                    }
            ...
            public void 建檔(out DataTable dtResult)
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    ...
                    Select 
                    單據='銷貨',憑證=saled.sano,單據日期=saled.sadate,訂單憑證=saled.orno,客戶編號=saled.cuno,業務編號=saled.emno,專案編號=saled.spno,送貨編號=saled.seno,倉庫名稱=saled.stname,saled.bomid,saled.ittrait
                    ,產品編號=saled.itno,品名規格=saled.itname,數量=saled.qty,單位=saled.itunit,售價=saled.price,折數=saled.prs,稅前售價=saled.taxprice,稅前金額=saled.mny,包裝數量=saled.itpkgqty,說明1=saled.memo
                    ,業務人員=SA.emname,送貨方式=SA.sename,專案名稱=SA.spname,結帳方式=SA.x4name,說明=SA.samemo
                    ,客戶簡稱=cust.cuname1,客戶名稱=cust.cuper1,客戶電話=cust.cutel1,客戶傳真=cust.cufax1,客戶類別=cust.cux1no
                    ,客戶類別名稱=X1.x1name
                    ,輔助編號=item.itnoudf,產品類別=item.kino
                    ...
                    (
                        Select saled.itno from saled " + skind + @" where 0=0 " + back + sQuery + @"
                        union all 
                        Select rsaled.itno from rsaled " + rkind + @" where 0=0 " + rback + rQuery + @"
        ...
        private void FrmNKind_Rpt_Load(object sender, EventArgs e)
        {
        ...
        {
            FrmNKind_Rpt.BackEnd = ckBackend.Checked;
            ...
            {
                new 成本計算(ckSale.Checked, ckRsale.Checked, sqlPara, sQuery, rQuery, ifkind, ifxx01).月平均成本(out dtResult);
            }
            ...
            //else if (radioT1.Checked)
            {
                ...
                using (var frm = new S_61.subMenuFm_4.FrmKind_Rptc(true))
                {
                    frm.flag = "類別排行榜";
